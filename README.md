...
- **Winrate tables** — the indicator shows an on-chart table scoring every historical signal, with two modes: **ATR Stop/Target** (default; each signal is scored by which bracket level — take profit or stop loss — is touched first, matching real TP/SL execution and the strategy script) or **Fixed Horizon** (win = price closed favorably N bars later). The strategy shows a live performance table (total trades, winrate, long/short breakdown, profit factor, net profit, max drawdown, open position).
